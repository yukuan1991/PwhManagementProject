...
void Widget::load(const QVariant &data)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        const auto jobContent = list.at(i).toMap()["作业内容"].toString();
        const auto basicTime = list.at(i).toMap()["基本时间"].toDouble();
        const auto rate = list.at(i).toMap()["宽放率"].toDouble();
        const auto stdTime = list.at(i).toMap()["标准时间"].toDouble();
        const auto valueAdded = list.at(i).toMap()["增值/非增值"].toString();
        const auto type = list.at(i).toMap()["操作分类"].toString();
