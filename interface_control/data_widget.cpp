...
int data_widget::row()
{
    assert(left_model_->rowCount() == right_model_->rowCount() and
           right_model_->rowCount() == result_model_->rowCount());

    return left_model_->rowCount();
}

void data_widget::set_no_editable()
{
...
json data_widget::json_data()try
{
...
void data_widget::save(const std::map<QString, QString> &additional_info, const QString& method_info) try
{
    auto iter = additional_info.find ("作业内容");
    assert (iter!=additional_info.end());
    auto filename = iter->second;
    json extra_info = map_to_json (additional_info);

    auto pts = json_data ();

    json sheet = json::object();
    sheet ["信息总览"] = info_pandect ();
    sheet ["预定工时法详细信息"] = pts;
    json& detail = sheet ["预定工时法详细信息"];
    assert (detail.is_object ());
    detail ["方法"] = method_info.toStdString ();
    ...
    save_task (filename, additional_info, working_procedure);
}
catch (std::exception &e)
{
    qDebug () << __LINE__ << e.what();
...
void data_widget::set_row(int num)
{
    ...
    left_model_->resize (num);
    ui->table_left->setModel(nullptr);
    ui->table_left->setModel(left_model_.get ());

    right_model_->resize (num);
    ui->table_right->setModel(nullptr);
    ui->table_right->setModel(right_model_.get ());
    result_model_->resize (num);
    ui->table_result->setModel(nullptr);
    ui->table_result->setModel(result_model_.get ());
}

void data_widget::add_row(int num)
{
    auto current_row = row ();
    set_row (current_row + num);
}

void data_widget::reduce_row(int num)
{
    auto current_row = row ();
    set_row (current_row - num);
}
...
json data_widget::map_to_json(const std::map<QString, QString> &map)
{
    try
    {
...
json data_widget::info_pandect()
{
    try
    {
        json data = json_data ();
        auto iter = data.find ("总计");
        ...
        {
            auto & row_object = (*iter)[i];

            auto task = row_object.find ("作业内容");
            assert (task != row_object.end ());
            assert (task->is_string ());

            content ["作业内容"] = *task;
            std::string str_total_code;

        auto code_iter = row_object.find ("代码");
        assert (code_iter != row_object.end ());
        assert (code_iter->is_array ());

        for (unsigned j=0; j < code_iter->size (); ++j)
        {
            auto single_code = (*code_iter)[j];
            assert (single_code.is_string ());
            std::string code = single_code;

            auto index = code.find_first_of ("_");
            assert (index != std::string::npos and index < code.length () - 1);
            auto sub_code = code.substr (index + 1);

            if (j != 0)
            {
                str_total_code += "." + sub_code;
            }
            else
            {
                str_total_code += sub_code;
        ...
        content ["预定工时法总代码"] = str_total_code;
        json::iterator iter;

        iter = row_object.find ("数量*频次"); assert (iter != row_object.end ());
        content ["数量*频次"] = *iter;
        iter = row_object.find ("评比系数"); assert (iter != row_object.end ());
        content ["评比系数"] = *iter;
        iter = row_object.find ("基本时间"); assert (iter != row_object.end ());
        content ["基本时间"] = *iter;
        iter = row_object.find ("宽放率"); assert (iter != row_object.end ());
        content ["宽放率"] = *iter;
        iter = row_object.find ("标准时间"); assert (iter != row_object.end ());
        content ["标准时间"] = *iter;
        iter = row_object.find ("增值/非增值"); assert (iter != row_object.end ());
        content ["增值/非增值"] = *iter;
        iter = row_object.find ("操作分类"); assert (iter != row_object.end ());
        content ["操作分类"] = *iter;
...
void data_widget::save_task(const QString& filename, const std::map<QString, QString> &info, const json& data)
{
